            ...
            <table class="admin-table">
                ...
                <tbody>
                    ${clientsList.map(client => {
                        const banks = client.bankAccounts.map(b => `${b.bankName}: ${b.accountNumber}`).join('<br>') || 'няма';
                        return `
                            <tr>
                                <td><code>${client.clientNumber}</code></td>
                                <td><strong>${client.firstName} ${client.lastName}</strong><br><small style="color:var(--text-muted);">@${client.username}</small></td>
